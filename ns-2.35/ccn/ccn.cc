...
static class ccnAgentClass : public TclClass {
public:
	ccnAgentClass() : TclClass("Agent/CCN") {}
	TclObject* create(int, const char*const*) {
		return (new ccnAgent());
	}
} class_ccn_agent;

ccnAgent::ccnAgent() : id_(0), openfile(0)
{
	bind("packetSize_", &size_);
	
}

void ccnAgent::sendmsg(int nbytes, AppData* data, const char* flags)
{
	Packet *p;
	int n;
	char buf[100];
...
int ccnAgent::command(int argc, const char*const* argv)
{
