...
class my_TrafficTrace : public TrafficGenerator {
 public:
	my_TrafficTrace();
	int command(int argc, const char*const* argv);
...
static class my_TrafficTraceClass : public TclClass {
 public:
	my_TrafficTraceClass() : TclClass("Application/Traffic/my_Trace") {}
	TclObject* create(int, const char*const*) {
	        return(new my_TrafficTrace());
	}
} class_my_traffictrace;

my_TrafficTrace::my_TrafficTrace()
{
	tfile_ = (my_TraceFile *)NULL;
}

void my_TrafficTrace::init()
{
	if (tfile_) 
		ndx_ = tfile_->setup();
}

int my_TrafficTrace::command(int argc, const char*const* argv)
{
...
double my_TrafficTrace::next_interval(int& size)
{
